	...
	for(j = 0; j < splitArray.length; j++)
	{
	 ...
	 else
	 {
		if(j==0&&document.getElementById("text_dropdown").value != "" )
		{
			...
			new_html+='<a style="background:#B3D4FF" id="first_link" href=\'javascript:ChooseLocation("'+location[0]+'","'+location[1]+'")\' onmouseover=\'javascript:OverLocation("'+location[0]+'","'+location[1]+'")\'>'+location[0]+'</a>';
		}
		else
		{
...
var combodropimage='images/ad_ac.jpg';
...
function transform_select(selectid, selectwidth, optionwidth, class_name)
{


	var selectbox=document.getElementById(selectid);
 
	document.write('<div id="dhtml_'+selectid+'" class="mselect" style="position:relative;top:'+(get_browser()=="Safari"?'-19':'-9')+'px;"><span id="selected_dropdown" style="font-size:11px;position:relative;left:-5px"><span style="color:white !important">.</span></span>');
	document.write('<input class="'+class_name+'" type="text" id="text_dropdown" name="text_dropdown" onblur = "field_lost_focus()" onkeyup="show_hint(this.value)" value="'+m_all+'" onmousedown="text_mousedown(this)" style="width:'+selectwidth+';" /><div id="location_dropdown" class="dropdown">');
	
	
	for (var i=0; i<selectbox.options.length; i++)
	{
		document.write('<a href=\'javascript:ChooseLocation("'+selectbox.options[i].text+'","'+selectbox.options[i].value+'")\' onmouseover=\'javascript:OverLocation("'+selectbox.options[i].text+'","'+selectbox.options[i].value+'")\'>'+selectbox.options[i].text+'</a>');
	}
