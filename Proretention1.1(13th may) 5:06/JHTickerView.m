...
-(void)setupView {
	...
	tickerLabel = [[UILabel alloc] initWithFrame:CGRectMake(5, 10, self.frame.size.width, self.frame.size.height)];
	[tickerLabel setBackgroundColor:[UIColor clearColor]];
	[tickerLabel setNumberOfLines:2];
...
-(void)animateCurrentTickerString
{
	...
	CGSize textSize = [currentString sizeWithFont:tickerFont constrainedToSize:CGSizeMake(self.frame.size.width,9999 ) lineBreakMode:UILineBreakModeWordWrap];
    ...
    switch (direction) {
        case JHTickerDirectionRTL:
            startingX = -textSize.height;
            endX = self.frame.size.height;
            break;
        case JHTickerDirectionLTR:
        default:
            startingX = self.frame.size.height;
            endX = -textSize.height;
            break;
    }

	// Set starting position
	[tickerLabel setFrame:CGRectMake( tickerLabel.frame.origin.x,startingX, textSize.width, textSize.height)];
	
	// Set the string
	[tickerLabel setText:currentString];
		
	// Calculate a uniform duration for the item	
	float duration = (self.frame.size.height) / tickerSpeed;
